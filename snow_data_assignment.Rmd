...
our_meteor_reader <- function(file){
  ...
  df <- read.delim(file,header = FALSE, sep = "", col.names = headers) %>% 
    select(year, month, air.temp..K.) 
...
temp_yearly <- meteor_data_full %>%
  group_by(year, 'air temp [K]') %>%
  summarize(mean_temp = mean(air.temp..K., na.rm=T))

ggplot(temp_yearly, aes(x = year, y = mean_temp, color = 'air temp [K]')) +
  geom_point() +
  ggthemes::theme_few() +
  ggthemes::scale_color_few() 
```
